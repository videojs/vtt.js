...
var processCues = function(window, cues, overlay, regions) {
  ...
  for (var i = 0; i < regions.length; i++) {
    var regionDisplay = regions[i].displayState.firstChild;
  ...
  (function() {
    var styleBox, cue, processedRegionCue = false;
    var regionCueCounts = {};
    ...
    for (var i = 0; i < cues.length; i++) {
      ...
      if (cue.region) {
        ...
        if (!(regionId in regionCueCounts)) {
          regionCueCounts[regionId] = 0;
        } else {
          regionCueCounts[regionId]++;
        }
        containerBox = handleRegionCue(cue, styleBox, regionCueCounts[regionId])
        processedRegionCue = true;
      } else {
        paddedOverlay.appendChild(styleBox.div);
      }

      if (cue.region.id === 'fred') {
        console.log(cue.text, containerBox);
      }
